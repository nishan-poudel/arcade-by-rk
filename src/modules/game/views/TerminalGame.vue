        ...
        <main class="game-area" data-test="game-area">
          ...
          >
            ...
            <div class="dpad" aria-label="Directional controls">
              <button
                class="dpad-btn dpad-up" aria-label="Move up"
                @click="movePlayer(-1, 0)">
                &#x25B2;
              </button>
              <button
                class="dpad-btn dpad-left" aria-label="Move left"
                @click="movePlayer(0, -1)">
                &#x25C0;
              </button>
              <button
                class="dpad-btn dpad-right" aria-label="Move right"
                @click="movePlayer(0, 1)">
                &#x25B6;
              </button>
              <button
                class="dpad-btn dpad-down" aria-label="Move down"
                @click="movePlayer(1, 0)">
                &#x25BC;
              </button>
            </div>
          ...
          <!-- LEVEL START cinematic overlay -->
          <Transition name="level-start">
            <div
              v-if="showLevelStart"
              class="ls-overlay"
              data-test="level-start"
              aria-live="polite"
            >
              <div class="ls-grid-lines">
                <span class="ls-line ls-line--h" />
                <span class="ls-line ls-line--v" />
              </div>
              <p class="ls-sector">SECTOR {{ level }}</p>
              <p class="ls-title">LEVEL {{ level }}</p>
              <p class="ls-sub">GRID LOADED · {{ levelTotalPackets }} PACKETS DETECTED</p>
              <div class="ls-bar-wrap"><div class="ls-bar" /></div>
              <p class="ls-go">COMMENCE RAID</p>
            </div>
          </Transition>

          <!-- LEVEL COMPLETE cinematic overlay -->
          <Transition name="level-complete">
            <div
              v-if="showLevelComplete"
              class="lc-overlay"
              data-test="level-complete"
              aria-live="polite"
            >
              <div class="lc-ring lc-ring--1" />
              <div class="lc-ring lc-ring--2" />
              <div class="lc-ring lc-ring--3" />
              <div class="lc-icon">✓</div>
              <p class="lc-title">PACKETS SECURED</p>
              <p class="lc-level">LEVEL {{ level }} COMPLETE</p>
              <p class="lc-score">+{{ levelScore }} pts</p>
              <p class="lc-next">Initiating next sector…</p>
            </div>
          </Transition>

          <!-- FIREWALL HIT cinematic overlay -->
          <Transition name="fw-hit">
            <div
              v-if="showFirewallHit"
              class="fw-overlay"
              data-test="firewall-hit"
              aria-live="assertive"
            >
              <div class="fw-static" />
              <div class="fw-scanline" />
              <div class="fw-icon">⚠</div>
              <p class="fw-title" data-glitch="FIREWALL BREACH">FIREWALL BREACH</p>
              <p class="fw-sub">Connection terminated.</p>
              <div class="fw-bars">
                <span class="fw-bar" /><span class="fw-bar" /><span class="fw-bar" />
                <span class="fw-bar" /><span class="fw-bar" />
              </div>
            </div>
          </Transition>
        </main>
...
const LS_KEY = 'gridRaider_highScore'
...
const showFirewallHit = ref(false)
...
const   levelFlashTimer: ReturnType<typeof setTimeout> | null = null
let   levelCompleteTimer: ReturnType<typeof setTimeout> | null = null
let   levelStartTimer: ReturnType<typeof setTimeout> | null = null
let   firewallHitTimer: ReturnType<typeof setTimeout> | null = null
...
function advanceLevel() {
  // Show cinematic level-complete overlay, then load next level
  levelScore.value = levelTotalPackets.value
  ...
  levelCompleteTimer = setTimeout(() => {
    ...
    // Show "level starting" animation over the new grid
...
function triggerGameOver() {
  ...
  // Show firewall-hit cinematic, then transition to GAMEOVER
  showFirewallHit.value = true

  if (firewallHitTimer) { clearTimeout(firewallHitTimer) }
  firewallHitTimer = setTimeout(() => {
    showFirewallHit.value = false
...
function movePlayer(dr: number, dc: number) {
  ...
  // Block input during transition cinematics
  if (showLevelComplete.value || showFirewallHit.value || showLevelStart.value) { return }
  ...
  if (target === 'X') {
    // hit firewall → mark collision cell, keep board visible
    next[row][col] = '0'
    next[nr][nc]   = 'H'   // special "hit" cell for visual feedback
    grid.value = next
