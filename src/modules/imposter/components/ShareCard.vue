<template>
  <!--
    Off-screen, fixed-size card that gets rasterised to a PNG for the "save
    scores" button. Hard-coded brand colours (not theme tokens) so the exported
    image always looks the same regardless of the viewer's light/dark setting.
  -->
  <div
    ref="root"
  ...
  >
    <div style="text-align: center; margin-bottom: 20px;">
      <div
        style="font-family: 'Fredoka Variable', system-ui, sans-serif; font-weight: 700;
    ...
    <template v-if="result">
      <div
        ...
        :style="{ background: crewWon ? '#d9f0e5' : '#fbe0dc' }"
      >
        <div
          style="font-family: 'Fredoka Variable', system-ui, sans-serif; font-weight: 700; font-size: 24px;"
          :style="{ color: crewWon ? '#2f6f56' : '#c0392b' }"
        >
          {{ crewWon ? 'Crew Win!' : 'Imposter Wins!' }}
        </div>
        <div style="font-size: 15px; margin-top: 6px; color: #3a333a;">
          The word was <b>{{ result.word }}</b>
          <template v-if="result.imposterHint"> · imposter saw “{{ result.imposterHint }}”</template>
        </div>
        <div style="font-size: 14px; margin-top: 4px; color: #c0392b;">
          {{ result.imposterNames.length === 1 ? 'Imposter: ' : 'Imposters: ' }}{{ result.imposterNames.join(', ') }}
        </div>
      </div>
      ...
      <div
        ...
        :key="row.playerId"
        style="display: flex; align-items: center; gap: 14px; padding: 11px 14px; border-radius: 16px;
               background: #fff; border: 2px solid #ece4e9; margin-bottom: 7px;"
      >
        ...
        <span style="flex: 1; font-weight: 600; font-size: 17px;">
          {{ row.name }}
          <span v-if="row.isImposter" style="color: #c0392b; font-size: 12px; font-weight: 700;"> · imposter</span>
        </span>
        <span
    ...
    <template v-else>
      <div style="font-size: 12px; text-transform: uppercase; letter-spacing: 0.12em; color: #6b5f68; margin-bottom: 8px;">
        Standings{{ gameNumber ? ` · ${gameNumber} game${gameNumber === 1 ? '' : 's'}` : '' }}
      </div>
      <div
      ...
      >
        ...
        <span style="font-family: 'Fredoka Variable', system-ui, sans-serif; font-weight: 700; font-size: 20px; color: #e8695a;">
          {{ p.score }}
        </span>
      </div>
    </template>

    <div style="text-align: center; font-size: 12px; color: #a99fa6; margin-top: 14px;">
      {{ stamp }}
    </div>
  </div>
</template>
...
import type { GameResult, PublicPlayer } from '../types/index.js'
