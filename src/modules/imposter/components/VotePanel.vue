<template>
  <!--
    Pinned voting panel — lives in the top block OUTSIDE the scroll area during
    discussion, so the vote is always reachable without scrolling. Two-step:
    pick a name → Submit → locked (no changes after).
  -->
  <div class="px-4 pt-1 shrink-0">
    <div class="w-full max-w-md mx-auto rounded-2xl border-2 border-warning/40 bg-card shadow-pop p-3">
      <div class="flex items-center justify-between mb-2.5">
        <p class="text-sm font-display font-bold text-warning">
          {{ locale.imposter.votePanel.prompt(gameState.voteRound) }}
        </p>
      ...
      <p
        v-if="amEliminated"
        class="text-center text-sm text-muted-foreground py-3 flex items-center justify-center gap-1.5"
      >
        <DoorOpen class="size-4" />{{ locale.imposter.votePanel.spectating }}
      </p>
      ...
      >
        <p class="text-sm font-display font-bold text-flavor-melon-ink flex items-center justify-center gap-1.5">
          <Check class="size-4" />{{ locale.imposter.votePanel.lockedFor(lockedName) }}
        </p>
        <p class="text-[11px] text-flavor-melon-ink/70 mt-0.5">{{ locale.imposter.votePanel.waitingOthers }}</p>
      </div>
      ...
      <template v-else>
        <div class="grid grid-cols-2 gap-2 max-h-[38vh] overflow-y-auto scroll-area -mr-1 pr-1">
          ...
          >
            <Vote v-if="selection === p.id" class="size-4" />
        ...
        <Button
          ...
          @click="$emit('submit')"
        >
          <Check class="size-5" />{{ locale.imposter.votePanel.submit }}
        </Button>
        <p class="text-[11px] text-muted-foreground/70 text-center mt-1.5">
          {{ selection ? locale.imposter.votePanel.confirmHint : locale.imposter.votePanel.pickHint }}
        </p>
      </template>

      <!-- Host: force the round -->
      <button
        v-if="isHost"
        type="button"
      ...
      >
        <Zap class="size-3.5" />{{ locale.imposter.votePanel.forceVote }}
      </button>
    </div>
  </div>
</template>
...
import { Check, DoorOpen, Vote, Zap } from '@lucide/vue'
...
const props = defineProps<{
  gameState: GameState
  myId: string
  /** Locally-picked candidate, pre-submit */
  selection: string
  /** Submitted (locked) candidate id, '' if not yet submitted */
  myVote: string
  isHost: boolean
}>()
...
const activeCount = computed(() => props.gameState.players.filter((p) => !p.eliminated).length)
const votedCount = computed(
  () => props.gameState.players.filter((p) => !p.eliminated && p.hasVoted).length,
)
const votablePlayers = computed(() =>
  props.gameState.players.filter((p) => p.id !== props.myId && !p.eliminated),
)
