...
describe('GameRoom full round → scoring → next round', () => {
  it('sweeps 13 tricks with a contrived all-spades hand and scores correctly', async () => {
    ...
    // Bids: seat0 calls 13 (will make it exactly), others call 1 (will miss).
    await bidAll(sockets, [13, 1, 1, 1])
    ...
    expect(state?.roundHistory[0].points).toEqual([13, -1, -1, -1])
    expect(state?.totals).toEqual([13, -1, -1, -1])
