<template>
  <div v-if="state" class="mx-auto flex w-full max-w-sm flex-col gap-6 animate-slide-up">
        ...
        >
          <template v-if="state.players[seat - 1]">
            <span class="font-display font-semibold">
              ...
              <span v-if="state.players[seat - 1]!.id === score.myPlayerId.value" class="text-muted-foreground">
                ({{ t.common.online }})
              </span>
            </span>
            <span class="flex items-center gap-2">
              ...
              />
              <Button
                v-if="score.isHost.value && !state.players[seat - 1]!.connected"
                size="sm"
                variant="ghost"
                @click="score.removePlayer(seat - 1)"
              >
    ...
    <Button v-if="score.isHost.value" size="lg" :disabled="seatedCount < 4" @click="score.startGame()">
...
function onRoundCount(value: string) {
  score.setRoundCount(Number(value) as RoundCount)
}
</script>
