...
This starts both the API Worker (`wrangler dev`, http://localhost:8787, the
real Workers + Durable Object runtime running locally via `workerd`, no
...
One person creates the room, the other three join with the room code. Same
idea for `http://localhost:5173/score` to try the in-person score keeper.
...
(including the must-trump-if-void edge case), and the scoring formula.
`api`'s tests drive real WebSocket connections against the actual Durable
Object classes, covering the full lobby-to-bidding-to-trick-play-to-scoring
loop and the score-keeper's round-entry validation. No mocks.
...
Both are set via `env.production.name` in each `wrangler.jsonc`. Worker names
are what determine the `*.workers.dev` URL, so renaming either one (e.g. to
move onto a custom domain later) means updating that file plus
...
auto-detection instead of using your build/deploy commands. That's exactly
what goes wrong if this is set incorrectly: it'll pick up the *other* app's
...
6. **No secrets to set.** This app has no login, no email, no PII, so
   there's nothing to `wrangler secret put`.

To redeploy after code changes, repeat steps 2 and 3 (CORS only needs
updating if the web Worker's URL ever changes).
